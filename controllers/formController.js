...
exports.postForm = (req, res) => {
  ...
  const { middlename, homeaddress, country, state, city, zip, birthdate, status, gender, hobby } = req.body;

  if (typeof hobby === 'string' || hobby instanceof String) req.body.hobby = [hobby]

  const sql =
    "INSERT INTO formtable (lastname, firstname, middlename, homeaddress, country, state, city, zip, birthdate, status, gender, hobby, a, b) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
  const values = [lastname, firstname, middlename, homeaddress, country, state, city, zip, birthdate, status, gender, hobby.join(), a, b];
